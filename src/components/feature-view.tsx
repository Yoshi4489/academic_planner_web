...
function Dashboard({authenticated,authReady}:{authenticated:boolean;authReady:boolean}){
  ...
  const tasks=useQuery({queryKey:["planner","tasks"],enabled:authReady&&authenticated,queryFn:()=>apiFetch("planner/tasks").then((data)=>listFrom<AcademicTask>(data))});
  const all=terms.data??[];const latest=[...all].sort((a,b)=>b.year-a.year||b.term_no-a.term_no)[0];const latestGpa=latest?.gpas?.[0];
  const actual=latestGpa?.cum_gpa??latestGpa?.gpa??0;const projected=latestGpa?.projected_gpa??actual;const credits=all.flatMap((term)=>term.courses??[]).filter((course)=>course.type==="ACTUAL").reduce((sum,course)=>sum+course.credit,0);
  const chart=[...all].sort((a,b)=>a.year-b.year||a.term_no-b.term_no).map((term)=>({name:`${term.year}/${term.term_no}`,actual:term.gpas?.[0]?.gpa??0,projected:term.gpas?.[0]?.projected_gpa??0}));
  if(!authReady)return <div className="space-y-5"><div className="skeleton h-40 rounded-[2rem]"/><div className="grid-auto"><div className="skeleton h-36 rounded-3xl"/><div className="skeleton h-36 rounded-3xl"/><div className="skeleton h-36 rounded-3xl"/></div></div>;
