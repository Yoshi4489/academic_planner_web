...
export function DataView({authenticated,authReady}:{authenticated:boolean;authReady:boolean}){
  ...
  if(!authReady)return <div className="skeleton h-96 rounded-3xl"/>;
  const counts=snapshot?transferCounts(snapshot):null;
  return <div className="space-y-6"><header className="card p-6 sm:p-8"><p className="eyebrow">DATA PORTABILITY</p><h2 className="text-3xl sm:text-4xl font-black mt-2">{th?"ข้อมูลเป็นของคุณ":"Your data stays yours"}</h2><p className="text-[var(--muted)] mt-2">{th?"สำรอง ย้าย และรวมข้อมูล Academic Planner ด้วย JSON schema version 1":"Back up, move, and merge Academic Planner data using JSON schema version 1"}</p></header>
    {message&&<div role="status" className="rounded-2xl border border-green-200 bg-green-50 p-4 text-green-900 flex gap-3"><CheckCircle2 className="shrink-0" size={19}/><span>{message}</span></div>}{error&&<div role="alert" className="rounded-2xl border border-red-200 bg-red-50 p-4 text-red-900 flex gap-3"><AlertTriangle className="shrink-0" size={19}/><span>{error}</span></div>}
    <section className="grid lg:grid-cols-2 gap-5"><article className="card p-6"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#eaf8c8]"><CloudDownload/></span><h3 className="text-xl font-black mt-5">{th?"ดาวน์โหลดข้อมูล":"Download your data"}</h3><p className="text-sm text-[var(--muted)] mt-2">{authenticated?(th?"รวมเทอม วิชา เป้าหมาย งาน ตารางเรียน และหมวดหลักสูตรจากคลาวด์":"Includes cloud semesters, courses, goals, tasks, schedule, and requirements"):(th?"ส่งออกเทอม วิชา และเป้าหมายที่เก็บบนอุปกรณ์นี้":"Exports semesters, courses, and goals stored on this device")}</p><button className="button-primary w-full mt-6" disabled={pending} onClick={()=>void exportData()}>{pending?<LoaderCircle className="animate-spin" size={18}/>:<CloudDownload size={18}/>} {th?"ดาวน์โหลด JSON":"Download JSON"}</button></article>
      <article className="card p-6"><span className="grid h-12 w-12 place-items-center rounded-2xl bg-[#ffebe6]"><Upload/></span><h3 className="text-xl font-black mt-5">{th?"อัปโหลดและตรวจสอบ":"Upload & validate"}</h3><p className="text-sm text-[var(--muted)] mt-2">{th?"ไฟล์จะถูกตรวจ schema และแสดงความขัดแย้งก่อนเปลี่ยนข้อมูล":"The file is validated and conflicts are shown before data changes"}</p><label className={`button-secondary w-full mt-6 cursor-pointer ${pending?"pointer-events-none opacity-50":""}`}><FileJson size={18}/>{th?"เลือกไฟล์ JSON":"Choose JSON file"}<input className="sr-only" type="file" accept="application/json,.json" disabled={pending} onChange={(event)=>void chooseFile(event.target.files?.[0])}/></label></article></section>
